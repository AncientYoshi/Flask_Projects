    ...
    if request.method == "POST":
        ...
        if s_item_object:
            if current_user.can_sell(s_item_object):
                ...
                flash(
                    f"Something went wrong with your sellings {s_item_object.name}. Please try again",
                    category="danger",
                )
...
def register_page():
    ...
    if form.validate_on_submit():
        ...
        flash(
            f"Account created successfully.Now you are logged in as {user_to_create.username}",
            category="success",
        )

        return redirect(url_for("market_page"))
    if form.errors != {}:  # if there are no errors in validation
...
def login_page():
    ...
    if form.validate_on_submit():
        ...
        if attempted_user and attempted_user.check_password_correction(
            ...
            flash("Invalid Password and Username.. Try again", category="danger")
